...
It then starts a container running a Jupyter Server and exposes the container's internal port `6888` to port `10000` of the host machine:

```bash
docker run -p 10000:6888 jupyter/scipy-notebook:6b49f3337709
```

You can modify the port on which the container's port is exposed by [changing the value of the `-p` option](https://docs.docker.com/engine/reference/run/#expose-incoming-ports) to `-p 6888:6888`.
...
```bash
docker run -it --rm -p 10000:6888 -v "${PWD}":/home/jovyan/work jupyter/datascience-notebook:6b49f3337709
```
